...
def test_one_good():
    total = grade([
        T(1.0, "True is True", "True", True, expectedValType=bool)
    ])
    assert total == 1.0
...
def test_one_bad():
    total = grade([
        T(1.0, "False is True", "False", True, expectedValType=bool)
    ])
    assert total == 0.0
...
def test_error_out():
    total = grade([
        T(1.0, "1/0 errors", "1/0", expectFail=True),
        T(1.0, "1/0 errors", "1/0", 0.0)
    ])
    assert total == 1.0
